...
if(isset($_POST['save_all'])){
    ...
    $availability = mysqli_real_escape_string($conn,$_POST['productAvailability']);
    $stock_mgmt = mysqli_real_escape_string($conn,$_POST['stock_management_type']);
   ...
   mysqli_query($conn,"UPDATE products SET 
    ...
    if(!empty($_POST['variant_id'])){
        ...
        foreach($_POST['variant_id'] as $k=>$vid){
            ...
            if(in_array($vid, $delete_variants)){
                ...
                $vl = mysqli_real_escape_string($conn,$_POST['variant_label'][$k]);
    ...
    if(!empty($_POST['new_variant_label'])){
        $vl = mysqli_real_escape_string($conn,$_POST['new_variant_label']);
    ...
    if($stock_mgmt == 'dependent'){
        $rs = mysqli_query($conn,"SELECT SUM(stock) as total FROM product_variants WHERE product_id=$id");
...
// ---------------- Fetch Variants ----------------
$variants=[];
$q=mysqli_query($conn,"SELECT * FROM product_variants WHERE product_id=$id");
...
<h2>🔧 Admin Product Editor</h2>
  <a href="../manage-products.php" class="btn btn-sm btn-warning">go back to manage products</a>
  <a href="index.php" class="btn btn-sm btn-warning">go back to manage products variants</a>
